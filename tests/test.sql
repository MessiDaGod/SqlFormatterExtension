...
IF OBJECT_ID(
    'PivotResults14a053ff-f900-4bc6-860f-2bd1c1a526c2'
) IS NOT NULL
...
IF OBJECT_ID(
    'ConcessionsAgg14a053ff-f900-4bc6-860f-2bd1c1a526c2'
) IS NOT NULL
...
IF OBJECT_ID(
    'FutureTenants14a053ff-f900-4bc6-860f-2bd1c1a526c2'
) IS NOT NULL
...
IF OBJECT_ID(
    'FreeLedgerMonthly14a053ff-f900-4bc6-860f-2bd1c1a526c2'
) IS NOT NULL
DROP TABLE [FreeLedgerMonthly14a053ff-f900-4bc6-860f-2bd1c1a526c2];


IF OBJECT_ID(
    'LabeledFreeFromLedger14a053ff-f900-4bc6-860f-2bd1c1a526c2'
) IS NOT NULL
DROP TABLE [LabeledFreeFromLedger14a053ff-f900-4bc6-860f-2bd1c1a526c2];


IF OBJECT_ID(
    'LedgerMonthly14a053ff-f900-4bc6-860f-2bd1c1a526c2'
) IS NOT NULL
DROP TABLE [LedgerMonthly14a053ff-f900-4bc6-860f-2bd1c1a526c2];


IF OBJECT_ID(
    'CurrentMonthlyChargesAndProration14a053ff-f900-4bc6-860f-2bd1c1a526c2'
) IS NOT NULL
DROP TABLE [CurrentMonthlyChargesAndProration14a053ff-f900-4bc6-860f-2bd1c1a526c2];


IF OBJECT_ID(
    'LabeledFuture14a053ff-f900-4bc6-860f-2bd1c1a526c2'
) IS NOT NULL
...
SELECT
    ...
    dtChargePost = CASE WHEN TRY_CONVERT(DATE, '') IS NULL
        OR YEAR(TRY_CONVERT(DATE, '')) = 1900 THEN lo.dtChargePost
    ...
    WindowEnd12 = DATEADD(
        MONTH,
        11,
        DATEFROMPARTS(YEAR(x.EffDate), MONTH(x.EffDate), 1)
    )
INTO
    [PropCtx14a053ff-f900-4bc6-860f-2bd1c1a526c2]
FROM Property p
JOIN Lockout lo ON lo.hProp = p.hMy
CROSS APPLY (
    SELECT
    EffDate = CASE WHEN TRY_CONVERT(DATE, '') IS NULL
    OR YEAR(TRY_CONVERT(DATE, '')) = 1900 THEN lo.dtChargePost
    ELSE TRY_CONVERT(DATE, '')
    END
) AS x
...
CREATE NONCLUSTERED
INDEX [IX_PropCtx_Start_14a053ff-f900-4bc6-860f-2bd1c1a526c2] ON [PropCtx14a053ff-f900-4bc6-860f-2bd1c1a526c2] (StartMonth, WindowEnd12, dtChargePost);
...
WHERE 0 = 0
    ...
    AND ts.status IN ('Future', 'Applicant');
...
SELECT
    QUOTENAME('Month' + CAST(n AS VARCHAR(2))) AS ColName,
    n                          AS MonthIdx
INTO
    [PivotCols14a053ff-f900-4bc6-860f-2bd1c1a526c2]
FROM N
OPTION
    (MAXRECURSION 0);
...
CREATE TABLE [Labeled14a053ff-f900-4bc6-860f-2bd1c1a526c2] (
    ...
    [IsFreeRent] BIT NULL CONSTRAINT [PK_Labeled14a053ff-f900-4bc6-860f-2bd1c1a526c2] PRIMARY KEY NONCLUSTERED ([Id] ASC)
    WITH
        (
            PAD_INDEX = OFF,
...
CREATE INDEX IX_LabeledAll_tenant_month ON [Labeled14a053ff-f900-4bc6-860f-2bd1c1a526c2] (TenantId, ChargeCodeId, MonthIdx) INCLUDE (AmountPerMonth, IsFreeRent);
...
WITH
    Base AS (
        SELECT
            ...
            CASE WHEN t.iStatus IN (4) THEN t.dtMoveOut
                ELSE CASE WHEN cr.dtTo IS NULL THEN DATEADD(YEAR, 1, pc.dtChargePost)
                    ELSE COALESCE(
                        cr.dtTo,
                        EOMONTH(maxTrans.MonthStart),
                        t.dtMoveOut,
                        DATEADD(YEAR, 1, pc.dtChargePost)
                    )
                END
            END                                                 AS OrigToDate,
            MoveInAmt = (
                DAY(EOMONTH(t.dtLeaseFrom)) - (DAY(t.dtLeaseFrom) - 1)
            ) / CAST(DAY(EOMONTH(t.dtLeaseFrom))                AS FLOAT) * (cr.dEstimated),
            LeaseAmt = cr.dEstimated,
            CASE WHEN cr.dtTo IS NULL THEN 1
                ELSE 0
            END                                                 AS IsOpenEnded,
            TenantCapDate = COALESCE(
                t.dtMoveOut,
                cr.dtTo,
                DATEADD(YEAR, 1, pc.dtChargePost)
            )
...
OUTER APPLY (
    ...
    OR ct.iType IN (2, 3)
        ...
        WHERE cr.dEstimated <> 0
            ...
            AND ts.status IN ('Future', 'Applicant')
            AND (
                pc.dtChargePost BETWEEN cr.dtfrom AND ISNULL(cr.dtto, DATEADD(yyyy, 1000, GETDATE()))
                OR cr.dtTo IS NULL
            )
    )
SELECT
    *
INTO
    [Base14a053ff-f900-4bc6-860f-2bd1c1a526c2]
FROM
    Base;
...
WITH
    Renewals AS (
        SELECT
            ...
            END IsOpenEnded,
            (
                DAY(EOMONTH(lh.dtLeaseFrom)) - (DAY(lh.dtLeaseFrom) - 1)
            ) / CAST(DAY(EOMONTH(lh.dtLeaseFrom))                         AS FLOAT) * (cr.dEstimated) ProratedAmount,
            lh.*,
...
SELECT
    *
INTO
    [Renewals14a053ff-f900-4bc6-860f-2bd1c1a526c2]
FROM
    Renewals;
...
WITH
    FutureTenants AS (
        SELECT
            ...
            (
                DAY(EOMONTH(t.dtLeaseFrom)) - (DAY(t.dtLeaseFrom) - 1)
            ) / CAST(DAY(EOMONTH(t.dtLeaseFrom))                  AS FLOAT) * (ct.cLeaseAmt) ProratedAmount,
            ct.*,
            TenantCapDate = COALESCE(
                t.dtMoveOut,
                t.dtLeaseTo,
...
LEFT JOIN prospect pr ON pr.hmy = t.hProspect
    AND pr.sStatus IN ('Applied', 'Approved', 'Resident')
OUTER APPLY (
    SELECT
    m.hMy,
    m.hTenant,
            ...
            AND (
                (
                    ...
                    AND ISNULL(t.hProspect, 0) = (
                        SELECT
                            CASE WHEN t.istatus = 2 THEN ISNULL(t.hProspect, 0)
                                ELSE pr.hmy
                            END
                    )
                    AND DATEFROMPARTS(YEAR(t.dtMoveIn), MONTH(t.dtMoveIn), 1) BETWEEN pc.StartMonth AND pc.WindowEnd12
                )
            )
            AND t.hmyperson IN (277666)
            OR (
...
INNER JOIN property p ON p.hmy = t.hProperty
    ...
    ConcessionsAgg AS (
        SELECT
            MIN(c.PropertyId)          AS PropertyId,
...
CREATE UNIQUE CLUSTERED INDEX [CIX_ConcessionsAgg_14a053ff-f900-4bc6-860f-2bd1c1a526c2] ON [ConcessionsAgg14a053ff-f900-4bc6-860f-2bd1c1a526c2] (PropertyId, TenantId);
...
WITH
    Clipped AS (
        SELECT
            ...
            CASE WHEN iStatus = 4 THEN MIN(ToDate)
                ELSE MAX(ToDate)
            END               AS ToDate,
        ...
        FROM
            (
                SELECT
                    ...
                    ToDate = CASE WHEN t.iStatus = 4
                        AND t.dtMoveOut IS NOT NULL THEN DATEFROMPARTS(YEAR(t.dtMoveOut), MONTH(t.dtMoveOut), 1)
                        WHEN b.IsOpenEnded = 1 THEN CASE WHEN b.OrigFromDate > pc.WindowEnd12 THEN b.OrigFromDate
                            ELSE pc.WindowEnd12
                        END
                        ELSE CASE WHEN b.OrigToDate IS NULL THEN pc.WindowEnd12
                            WHEN EOMONTH(b.OrigToDate) < pc.WindowEnd12 THEN EOMONTH(b.OrigToDate)
                            ELSE pc.WindowEnd12
                        END
                    END,
                    b.MoveInAmt,
                    b.LeaseAmt,
                    0 IsRenew,
                    CASE WHEN t.iStatus = 4 THEN 1
                        ELSE 0
                    END IsNotice,
                    t.iStatus
                FROM
                    [Base14a053ff-f900-4bc6-860f-2bd1c1a526c2] b
...
JOIN [PropCtx14a053ff-f900-4bc6-860f-2bd1c1a526c2] pc ON pc.PropertyId = b.PropertyId
                ...
                SELECT
                    ...
                    RuleStart = b.OrigFromDate,
                    FromDate = pc.StartMonth,
                    ToDate = CASE WHEN b.IsOpenEnded = 1 THEN CASE WHEN b.OrigFromDate > pc.WindowEnd12 THEN b.OrigFromDate
                            ELSE pc.WindowEnd12
                        END
                    ...
                    t.iStatus
                FROM
                    [FutureTenants14a053ff-f900-4bc6-860f-2bd1c1a526c2] b
INNER JOIN tenant t ON t.hmyperson = b.TenantId
JOIN [PropCtx14a053ff-f900-4bc6-860f-2bd1c1a526c2] pc ON pc.PropertyId = b.PropertyId
                WHERE (
                ...
                SELECT
                    ...
                    t.iStatus
                FROM
                    [Renewals14a053ff-f900-4bc6-860f-2bd1c1a526c2] b
INNER JOIN tenant t ON t.hmyperson = b.TenantId
JOIN [PropCtx14a053ff-f900-4bc6-860f-2bd1c1a526c2] pc ON pc.PropertyId = b.PropertyId
...
SELECT
    *
INTO
    [MonthSeries14a053ff-f900-4bc6-860f-2bd1c1a526c2]
FROM
    MonthSeries;


/* ===== MonthSeries ===== */
CREATE CLUSTERED INDEX [CIX_MonthSeries_14a053ff-f900-4bc6-860f-2bd1c1a526c2] ON [MonthSeries14a053ff-f900-4bc6-860f-2bd1c1a526c2] (PropertyId, TenantId, ChargeCodeId, MonthStart);


CREATE NONCLUSTERED
INDEX [IX_MonthSeries_Rule_14a053ff-f900-4bc6-860f-2bd1c1a526c2] ON [MonthSeries14a053ff-f900-4bc6-860f-2bd1c1a526c2] (RuleStart, Source) INCLUDE (LeaseAmt, MoveInAmt);
...
CREATE CLUSTERED INDEX [CIX_FreeLedgerMonthly_14a053ff-f900-4bc6-860f-2bd1c1a526c2] ON [FreeLedgerMonthly14a053ff-f900-4bc6-860f-2bd1c1a526c2] (PropertyId, TenantId, MonthIdx);


CREATE NONCLUSTERED
INDEX [IX_FreeLedgerMonthly_Start_14a053ff-f900-4bc6-860f-2bd1c1a526c2] ON [FreeLedgerMonthly14a053ff-f900-4bc6-860f-2bd1c1a526c2] (TenantId, MonthIdx) INCLUDE (FreeAmt, MonthStart);
...
INTO
    [LabeledFreeFromLedger14a053ff-f900-4bc6-860f-2bd1c1a526c2]
FROM
    [FreeLedgerMonthly14a053ff-f900-4bc6-860f-2bd1c1a526c2] flm
JOIN [PropCtx14a053ff-f900-4bc6-860f-2bd1c1a526c2] pc ON pc.PropertyId = flm.PropertyId;


CREATE CLUSTERED INDEX [CIX_LabeledFreeFromLedger_14a053ff-f900-4bc6-860f-2bd1c1a526c2] ON [LabeledFreeFromLedger14a053ff-f900-4bc6-860f-2bd1c1a526c2] (PropertyId, TenantId, MonthIdx);
...
CREATE CLUSTERED INDEX [CIX_LedgerMonthly_14a053ff-f900-4bc6-860f-2bd1c1a526c2] ON [LedgerMonthly14a053ff-f900-4bc6-860f-2bd1c1a526c2] (PropertyId, TenantId, ChargeCodeId, MonthStart);


CREATE NONCLUSTERED
INDEX [IX_LedgerMonthly_Amt_14a053ff-f900-4bc6-860f-2bd1c1a526c2] ON [LedgerMonthly14a053ff-f900-4bc6-860f-2bd1c1a526c2] (TenantId, MonthStart) INCLUDE (LedgerAmt);
...
SELECT
    ...
    AmountPerMonth = CASE WHEN ms.MonthStart < ms.RuleStart THEN 0.0 /* FREE (65) should never come from CMP; ledger FREE is added later. */
        WHEN ms.ChargeCodeId = 65 THEN 0.0 /* For rent/other recurring, honor ledger override */
        WHEN lm.LedgerAmt IS NOT NULL THEN lm.LedgerAmt
        WHEN ms.MonthStart = ms.RuleStart THEN ms.MoveInAmt
        WHEN t.iStatus = 4
        AND t.dtMoveOut IS NOT NULL
        AND ms.MonthStart = DATEFROMPARTS(YEAR(t.dtMoveOut), MONTH(t.dtMoveOut), 1)
        AND t.dtMoveOut < EOMONTH(t.dtMoveOut) THEN ms.LeaseAmt * (
            DAY(t.dtMoveOut) * 1.0 / DAY(EOMONTH(t.dtMoveOut))
        )
        ELSE ms.LeaseAmt
    END
INTO
    [CurrentMonthlyChargesAndProration14a053ff-f900-4bc6-860f-2bd1c1a526c2]
FROM
    [MonthSeries14a053ff-f900-4bc6-860f-2bd1c1a526c2] ms
...
CROSS APPLY (
    SELECT
    MonthIdx = DATEDIFF(MONTH, pc.StartMonth, ms.MonthStart) + 1
)                                           AS ix
WHERE ms.Source = 'Base';


CREATE CLUSTERED INDEX [CIX_CMP_14a053ff-f900-4bc6-860f-2bd1c1a526c2] ON [CurrentMonthlyChargesAndProration14a053ff-f900-4bc6-860f-2bd1c1a526c2] (PropertyId, TenantId, ChargeCodeId, MonthStart);


CREATE NONCLUSTERED
INDEX [IX_CMP_View_14a053ff-f900-4bc6-860f-2bd1c1a526c2] ON [CurrentMonthlyChargesAndProration14a053ff-f900-4bc6-860f-2bd1c1a526c2] (MonthIdx) INCLUDE (AmountPerMonth, ChargeCodeId, ChargeCode);
...
SELECT
    ...
    ix.MonthIdx,
    MonthLabel = 'Month' + CAST(ix.MonthIdx AS VARCHAR(3)),
    AmountPerMonth = CASE WHEN ms.MonthStart < ms.RuleStart THEN 0.0
        WHEN ms.MonthStart = ms.RuleStart THEN ms.MoveInAmt
        ELSE ms.LeaseAmt
    END
INTO
...
WHERE ms.Source = 'Future'
    AND NOT EXISTS (
        SELECT
            1
        FROM
            [MonthSeries14a053ff-f900-4bc6-860f-2bd1c1a526c2] x
        WHERE x.PropertyId = ms.PropertyId
            ...
            AND x.Source IN ('Base', 'Renewal')
    );


CREATE CLUSTERED INDEX [CIX_LabeledFuture_14a053ff-f900-4bc6-860f-2bd1c1a526c2] ON [LabeledFuture14a053ff-f900-4bc6-860f-2bd1c1a526c2] (PropertyId, TenantId, ChargeCodeId, MonthIdx);


SELECT
    *
INTO
    [LabeledAll14a053ff-f900-4bc6-860f-2bd1c1a526c2]
FROM
    (
        SELECT
            lb.PropertyId,
            ...
            lb.MonthLabel,
            lb.AmountPerMonth,
            IsFreeRent = CASE WHEN lb.ChargeCode = 'free' THEN 1
                ELSE 0
            END
        FROM
            [CurrentMonthlyChargesAndProration14a053ff-f900-4bc6-860f-2bd1c1a526c2] lb
        UNION ALL
        SELECT
            lf.PropertyId,
            lf.PropertyCode,
            ...
            lf.MonthLabel,
            lf.AmountPerMonth,
            CASE WHEN lf.ChargeCode = 'free' THEN 1
                ELSE 0
            END
        FROM
            [LabeledFuture14a053ff-f900-4bc6-860f-2bd1c1a526c2] lf
    ) x;


CREATE CLUSTERED INDEX [CIX_LabeledAll_14a053ff-f900-4bc6-860f-2bd1c1a526c2] ON [LabeledAll14a053ff-f900-4bc6-860f-2bd1c1a526c2] (PropertyId, TenantId, ChargeCodeId, MonthIdx);
...
WITH
    ...
    RentPerMonth AS (
        SELECT
            ...
            la.AmountPerMonth
        FROM
            [LabeledAll14a053ff-f900-4bc6-860f-2bd1c1a526c2] la
        ...
        SELECT
            ...
            MonthIdx = DATEDIFF(
                ...
                DATEADD(
                    MONTH,
                    t.n - 1,
                    DATEFROMPARTS(YEAR(ca.dtMoveIn), MONTH(ca.dtMoveIn), 1)
                )
            ) + 1,
            n = t.n
        FROM
            [ConcessionsAgg14a053ff-f900-4bc6-860f-2bd1c1a526c2] ca
...
JOIN Tally t ON t.n <= 12
    ...
    CandidatesWindow AS (
        SELECT
            *
        FROM Candidates
        WHERE MonthIdx BETWEEN 1 AND 12  /* keep within the 12-mo report window */
    ),
    FreePostedBeforeWindow AS (
        SELECT
            PropertyId,
            TenantId,
    ...
    CandidatesWithRemaining AS (
        SELECT
            c.PropertyId,
            c.PropertyCode,
            c.TenantId,
            /* Remaining free after subtracting posts before the window */
            RemainingTotal = CASE WHEN c.TotalFreeRentAmount - ISNULL(fp.PostedBefore, 0) < 0 THEN 0
...
LEFT JOIN FreePostedBeforeWindow fp ON fp.PropertyId = c.PropertyId
    ...
    CandidatesWithCap AS (
        SELECT
            ...
            cw.n,
            cw.RemainingTotal,
            Cap = CASE WHEN ISNULL(flm.FreeAmt, 0) <> 0 THEN 0.0
...
LEFT JOIN [FreeLedgerMonthly14a053ff-f900-4bc6-860f-2bd1c1a526c2] flm ON flm.PropertyId = cw.PropertyId
    ...
    LedgerWindow AS (
        ...
        FROM
            [FreeLedgerMonthly14a053ff-f900-4bc6-860f-2bd1c1a526c2]
        WHERE MonthIdx BETWEEN 0 AND 12  /* Only in the report window */
    ),
    Running AS (
        SELECT
            cwc.PropertyId,
            ...
            RunningCap = SUM(cwc.Cap) OVER (
                PARTITION BY
                    ...
                    cwc.TenantId
                ORDER BY
                    cwc.n ROWS UNBOUNDED PRECEDING
            ),
            PrevRunningCap = SUM(cwc.Cap) OVER (
                PARTITION BY
                    ...
                    cwc.TenantId
                ORDER BY
                    cwc.n ROWS BETWEEN UNBOUNDED PRECEDING
                    AND 1 PRECEDING
            ),
            ...
            LedgerToDatePrev = COALESCE(
                (
                    SELECT
                        SUM(ABS(f.FreeAmt))
                    FROM
                        [FreeLedgerMonthly14a053ff-f900-4bc6-860f-2bd1c1a526c2] f
                    WHERE f.PropertyId = cwc.PropertyId
                        AND f.TenantId = cwc.TenantId
                        AND f.MonthIdx BETWEEN 1 AND cwc.MonthIdx  - 1
                ),
                0
            )
    ...
    ConcessionSpread AS (
        SELECT
            ...
            AmountPerMonth = - CASE WHEN RemainingTotal <= (LedgerToDatePrev + PrevRunningCap) THEN 0
                WHEN RemainingTotal >= (LedgerToDatePrev + RunningCap) THEN Cap
...
INSERT INTO
    [Labeled14a053ff-f900-4bc6-860f-2bd1c1a526c2] (
        PropertyId,
        PropertyCode,
        TenantId,
        ChargeCodeId,
...
WHERE s.AmountPerMonth <> 0
    AND NOT EXISTS (
        SELECT
            1
        FROM
            [LabeledFreeFromLedger14a053ff-f900-4bc6-860f-2bd1c1a526c2] L
    ...
    NeedFreeRow AS (
        ...
        WHERE NOT EXISTS (
                SELECT
                    1
                FROM
                    [Labeled14a053ff-f900-4bc6-860f-2bd1c1a526c2] L
...
SELECT
    @sumCols_Name = STUFF(
        (
            SELECT
                ',SUM(COALESCE(' + ColName + ', 0)) ' + ColName
            FROM
                [PivotCols14a053ff-f900-4bc6-860f-2bd1c1a526c2]
            ORDER BY
                MonthIdx
...
IF @cols IS NULL
OR @cols = '' BEGIN RAISERROR ('No month columns found to pivot.', 11, 1);
...
SELECT
    @sumCols_TotalExpr = STUFF(
        (
            SELECT
                '+SUM(COALESCE(' + ColName + ',0))'
            FROM
                [PivotCols14a053ff-f900-4bc6-860f-2bd1c1a526c2]
            ORDER BY
                MonthIdx
...
SELECT
    ...
    CASE WHEN r.IsFreeRent = 1 THEN 'Free Rent'
        ELSE ct.sName
    END AS ChargeName,
    'SCREEN' OutputType,
    u.hMy UnitId,
    1 Total,
    HasRenew = CASE WHEN lh.Id IS NOT NULL THEN 'Yes'
        ELSE ''
    END,
    lh.Id lhIds,
    pr.hmy ProspectId,
...
FROM
    [PivotResults14a053ff-f900-4bc6-860f-2bd1c1a526c2] r
